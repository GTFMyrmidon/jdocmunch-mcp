...
⚠ jDoc/jData difference from jcm: the disclosure is TOP-LEVEL, not under ``_meta``.
...
the same trap that forced the post-filter re-attach of the absence ref in
v1.117.0 and the budget block in v1.104.0.

⚠⚠ **jdoc got this LAST, in v1.124.1, and the gap was reported as #104.** The
v1.124.0 fix for that issue disclosed ignored arguments and did NOT degrade the
absence verdict, which is the half that actually matters here: jdoc mints
citable ``absent:<sha>`` refs (v1.117.0), so a call whose scoping argument was
silently dropped could reach ``absent`` and be cited as proof the target is not
there. Disclosure alone does not stop that; the downgrade does.
