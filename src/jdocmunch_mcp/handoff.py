...
HANDOFF_SCHEMA_V2 = "jdocmunch.handoff/v2"
...
def _render_absence_detail(ref: str, indent: str) -> list:
    ...
    if scope:
        ...
        lines.append(f"{indent}- scope: whole indexed corpus")
    scanned = rec.get("scanned") or {}
    if scanned:
        rendered = ", ".join(f"{k}={scanned[k]}" for k in sorted(scanned))
        lines.append(f"{indent}- scanned: {rendered}")
    ...
    *,
    ...
    if invalid_claims:
        return {
            ...
            "hint": (
                "Every claim-scoped ref must be a section id (or its doc path) "
    ...
    if not isinstance(evidence_refs, list) or (not evidence_refs and not claim_refs_flat):
        return {
            "error": (
                ...
                "or every claim must carry its own evidence_refs"
            )
        }
