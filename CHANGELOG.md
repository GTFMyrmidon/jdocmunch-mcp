...
## [1.97.0] - 2026-07-16 - cost_avoided corrected to current Opus pricing (was overstating 3x)

`storage/token_tracker.PRICING` carried a stale Opus input rate of $15/MTok
(the retired Opus 4.0/4.1 price). `PRICING` feeds `cost_avoided()`, which is
emitted in `_meta` on nearly every tool response, so every response's Opus
dollar figure was roughly 3x too high. Current Opus (4.8/4.7/4.6) is $5/MTok.
...
- `claude_sonnet` ($3/MTok) and `claude_haiku` ($1/MTok) entries, so
  `cost_avoided` / `total_cost_avoided` show the full current model set
  (parity with the sibling code MCP's price table). Additive keys only; the
  existing `claude_opus` and `gpt5_latest` keys are unchanged in name, so the
  wire shape stays 1.x-compatible (new keys additive, plus a value correction).
...
add/rename. Suite parity: jcm v1.108.130 (receipt table) + jdata v1.19.0 (same
token_tracker fix).
