"""v1.112.0 — tool-surface schema receipt in get_session_stats (jcm v1.108.153 parity).
...
at the meter's bytes/4 scale. Attached to get_session_stats as an advisory
`tool_surface` block; a helper failure omits the block, never fails the call.
jDoc has no Counter surface, so the block carries `profile` but no `surface`.
"""
...
class TestToolSurfaceStats:
    ...
    def test_narrow_profile_avoids_schema_tokens(self, monkeypatch):
        monkeypatch.setenv("JDOCMUNCH_TOOL_PROFILE", "core")
        stats = server._tool_surface_stats()
        assert stats["profile"] == "core"
        assert stats["visible_tools"] < stats["catalog_tools"]
    ...
    def test_disabled_tools_reduce_visible(self, monkeypatch):
        ...
        monkeypatch.setenv("JDOCMUNCH_DISABLED_TOOLS", victim)
        stats = server._tool_surface_stats()
