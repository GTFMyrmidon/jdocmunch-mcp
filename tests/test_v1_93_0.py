...
execution (Claude Code plan mode) run jDoc's query tools silently while still
prompting on the handful that index / mutate / delete a doc index.
"""
...
def test_representative_read_and_write_tools():
    ...
    for read_tool in ("get_doc", "search_sections", "get_section", "list_docs", "verify_index"):
        assert by_name[read_tool].annotations.readOnlyHint is True, f"{read_tool} should be read-only"
    for write_tool in ("index_local", "doc_index_repo", "delete_index", "define_repo_group"):
