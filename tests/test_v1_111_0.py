"""v1.111.0 — munch.runtime.identity/v1 resource (suite parity, jcodemunch-mcp#371)."""
...
def test_payload_carries_required_fields():
    ...
    assert payload["product"] == "jdocmunch-mcp"
...
def test_repeated_reads_are_stable(monkeypatch):
    monkeypatch.delenv("JDOCMUNCH_LAUNCH_ID", raising=False)
...
def test_launch_id_echoed_from_product_env(monkeypatch):
    monkeypatch.setenv("JDOCMUNCH_LAUNCH_ID", "harness-abc")
...
def test_two_processes_get_distinct_instance_ids():
    ...
    code = (
        "from jdocmunch_mcp import runtime_identity as r;"
...
def test_list_resources_advertises_identity():
    from jdocmunch_mcp.server import list_resources
...
def test_read_resource_returns_identity_json():
    from jdocmunch_mcp.server import read_resource
