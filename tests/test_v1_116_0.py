...
def _clear():
    ...
    handoff.note_served_rows(SERVED_ROWS)
    yield
    handoff.clear_handoffs()
    handoff.clear_session_record()
...
SERVED_ROWS = [
    {"id": "docs/architecture.md::storage-model", "doc_path": "docs/architecture.md"},
    {"id": "docs/api.md::authentication", "doc_path": "docs/api.md"},
    {"id": "docs/guide.md::install", "doc_path": "docs/guide.md"},
]
SERVED = [row["id"] for row in SERVED_ROWS]
...
def _finalize(sections, evidence_refs=None, **kw):
    return handoff.finalize_handoff(
        repo="owner/name",
        task="Audit the documentation surface",
        sections=sections,
...
class TestClaimScopedEvidence:
            ...
            [
                {
                    ...
                    "content": "Overview.",
                    "claims": [
                        _claim("runtime.exterior", "The storage model is documented.", [SERVED[1]]),
                        _claim("runtime.eval", "Authentication is documented.", [SERVED[0]]),
                    ],
                }
            ],
            evidence_refs=[SERVED[0]],
        )
        assert receipt["schema"] == "jdocmunch.handoff/v2"
    ...
    def test_evidence_renders_beside_the_claim(self):
            ...
            [
                {
                    ...
                    "claims": [
                        _claim(
                            "runtime.exterior",
                            "The storage model is documented.",
                            [SERVED[1]],
        ...
        body = handoff.get_handoff(receipt["handoff_id"])["body"]
        claim_at = body.index("### The storage model is documented.")
        ...
        assert "- Claim id: `runtime.exterior`" in body
    ...
    def test_statement_preserved_verbatim(self):
        statement = "The storage model lives in `docs/architecture.md` (see #377)."
        receipt = _finalize(
...
class TestFailsClosed:
    def test_unknown_claim_ref_names_its_claim(self):
            ...
            [
                {
                    ...
                    "claims": [
                        ...
                        _claim("c2", "Bad.", ["docs/never_served.md::ghost"]),
                    ],
                }
            ]
        )
        ...
        assert result["invalid_claims"] == [
            {"claim_id": "c2", "unknown_refs": ["docs/never_served.md::ghost"]}
        ]
